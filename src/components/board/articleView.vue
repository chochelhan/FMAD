<template>
    <div>
        <div class="view-header">
            ...
            <div class="author-box">
                <div>
                    ...
                    <span class="hit">조회수 : {{articleInfo.hit}}</span>
                </div>
        ...
        <div v-if="articleInfo.dfiles">
            <div style="padding:10px 0 0 30px;font-weight:bold;color:#000;font-size:12pt;">첨부 파일</div>
            <ul class="file-box">
                <li v-for="(item,index) in articleInfo.dfiles" @click="fileDown(index)">
                    {{item.name}}
                </li>
            </ul>
        </div>
        ...
        <div class="reple-layout" v-if="rlauth">
            <comment :parent="'board'" :parentNo="no" :rwauth="rwauth" :isAdmin="isAdmin"></comment>
...
<script>
	...
	export default {
		...
		methods: {
			...
			fileDown(index) {
				let path = this.articleInfo.dfiles[index];
				location.href = this.host+'/api/board/fileDown?path='+path.path;
            }
