        ...
        <div>
            <table class="list-table">
                <tbody
                       ...
                       :key="item.no" :class="item.gclass" @click="articleView(item,index)">
                <tr>
                    <td style="width:70px;">{{ item.num }}</td>

                    <td style="width:110px;color:#686EFB;" v-if="categoryUse" >[<span class="category-box">{{ categorys[item.category]}}</span>]</td>
                    <td style="text-align:left;padding-left:10px;">{{ item.subject }}</td>
                    <td style="width:50px;">
                        <i v-if="!item.contentView" class="fas fa-chevron-down"></i>
                        <i v-else class="fas fa-chevron-up"></i>
                    </td>
                </tr>
                <tr v-if="item.contentView">
                    <td colspan="4" class="contentHtml">
	...
	export default {
		...
		created() {
			...
			if (this.boardInfo.categoryUse == 'yes') {
				...
				for (let cate of this.boardInfo.categorys) {
					this.categorys[cate.code] = cate.name;
				}
			}
			this.getServerData();
		},
		methods: {
			...
			articleView(item,index) {
				let k = 0;
				for(let data of this.dataList) {
					if(item.no != data.no) {
						data.gclass = '';
						data.contentView = false;
						this.$set(this.dataList,k,data);
                    }
                    k++;
                }
				if(item.contentView) {
					item.gclass = '';
					item.contentView = false;
				} else {
					item.gclass = 'active';
					item.contentView = true;
                }
			...
			getServerData() {
				...
				if (this.articleList.length > 0) {
					...
					for (let item of this.serverDataList) {
						...
						item.contents = UTIL.nl2brContent(item.contents);
            ...
            tbody {
                ...
                &.active {
                    tr {
                        &:first-child {
                            td {
                                border-top: solid 2px #686EFB;
                                &:last-child {
                                    border-right: solid 2px #686EFB;
                                }
                                &:first-child {
                                    border-left: solid 2px #686EFB;
                                }
                            }
                        }
                        &:last-child {
